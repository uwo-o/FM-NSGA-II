...
\begin{itemize}
    ...
    \item \textbf{Generación 4: Spherical Harmonic Manifold (SHM-NSGA-II).} Rompiendo las limitaciones de un único eje direccional de AM, se extraen bases ortonormales tridimensionales $\mathbb{R}^3$ del espacio original $\mathbb{R}^D$ para luego deformarlas volumétricamente usando polinomios de Legendre asociados (Armónicos Esféricos reales). Esto recupera la flexibilidad estocástica del FM pero con la altísima velocidad inferencial del AM.
    \item \textbf{Generación 5: El Salto Diferencial (MGDA).} El hallazgo crítico de que AM y GAM son diferenciables permitió descartar la selección natural de NSGA-II y adoptar el \textbf{Multiple Gradient Descent Algorithm (MGDA)}. Al utilizar gradientes exactos junto con optimizadores modernos (Adam, Focal Loss), el tiempo de convergencia colapsó de decenas de segundos a fracciones de segundo, cristalizando el método.
\end{itemize}
...
Consideremos, a modo de ilustración, un conjunto de datos médicos en 30 dimensiones. En lugar de trazar una hiper-esfera compleja en 30D, el algoritmo AM descubre matemáticamente un único vector direccional "maestro" $\mathbf{v}$. Para evaluar a un nuevo paciente $\mathbf{x}$, el modelo computa qué ángulo forma el paciente con respecto a ese vector maestro. Luego, el polinomio de Chebyshev dicta la regla: \textit{"Si el paciente está a un ángulo de $45^\circ$, su radio límite máximo de pertenencia a la enfermedad es de 2.5 unidades"}. Hemos comprimido 30 dimensiones a una simple comprobación trigonométrica de un escalar.
...
En la práctica, supongamos un problema bidimensional donde predecimos Obesidad usando la Característica 1 (Perímetro Abdominal) y la Característica 2 (Color de Ojos). El modelo entrena un polinomio para $d=1$ y otro para $d=2$. Durante el entrenamiento, el motor de optimización descubre que el Color de Ojos ($d=2$) arroja ruido, por lo que castiga el multiplicador $w_2$ acercándolo a cero (ej. $w_2 = 0.001$). Al multiplicar el polinomio 2 por $0.001$, todo el término se anula en la sumatoria matemática. Así, al revisar el cromosoma final, vemos que $w_1 = 0.99$ y $w_2 = 0.001$, proveyendo una explicación analítica innegable de cómo el modelo descarta ejes espúreos al tomar sus decisiones.
...
\subsection{Generación 4: Spherical Harmonic Manifold (SHM)}
Como evolución natural del AM, el SHM levanta la restricción de poseer un solo eje de proyección polar. En su lugar, el cromosoma aprende una matriz ortonormal (mediante Gram-Schmidt ponderado) que forja una base tridimensional $\{ \mathbf{v}_1, \mathbf{v}_2, \mathbf{v}_3 \} \subset \mathbb{R}^D$. El espacio hiper-dimensional es proyectado y condensado localmente hacia una esfera $S^2$.

Sobre esa esfera virtual, en vez de un simple polinomio de Chebyshev, modelamos deformaciones volumétricas de frontera usando Armónicos Esféricos Reales ($Y_l^m$), obtenidos mediante polinomios asociados de Legendre. La ecuación de radio límite de frontera es:
\begin{equation}
    r^*(\theta, \phi) = \sum_{l=0}^{L} \sum_{m=-l}^{l} c_{l,m} \, Y_l^m(\theta, \phi)
\end{equation}
Al igual que en AM y GAM, la pertenencia recae únicamente en validar si la magnitud radial superó al umbral, pero brindando la inmensa invarianza rotacional y capacidad morfológica del FM (superando de esta manera las asimetrías de un simple eje). Su ventaja estructural yace en su cálculo asintótico $\mathcal{O}(D + L^2)$ inferencial sin requerir el cuello de botella que implica la sub-búsqueda del parámetro iterativo de punto mínimo.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{../build/plot_shm_chromosome.png}
    \caption{Representación del Cromosoma Spherical Harmonic Manifold (SHM). Evoluciona una base proyectiva tridimensional y hasta $(L+1)^2$ coeficientes esféricos morfológicos.}
    \label{fig:chrom_shm}
\end{figure}
...
Retomando el escenario médico anterior, supongamos que un nuevo Paciente X ingresa al sistema. El evaluador calcula la distancia de este paciente a los bordes de cada modelo:
...
Físicamente, este algoritmo actúa como un árbitro geométrico en el espacio de parámetros. Imaginemos que durante el entrenamiento, en la iteración 100, el vector gradiente del Error ($\nabla O_1$) le indica a los parámetros del polinomio: \textit{"Aumenten su valor hacia el Norte para abarcar un dato que se nos escapó"}. Sin embargo, el gradiente de Complejidad ($\nabla O_2$) reclama: \textit{"Si vamos al Norte, la curva se vuelve muy serpenteante e inestable; debemos ir al Sur"}. El MGDA computa el ángulo de conflicto entre ambos vectores y resuelve matemáticamente el punto de equilibrio óptimo $\mathbf{d}$ (ej. moverse hacia el Oeste), logrando que el modelo reduzca el error empírico sin deteriorar irreparablemente la integridad de la curva topológica.

\section{Evaluación Experimental Integral}

\input{tables.tex}

\subsection{Validación Estadística (Wilcoxon)}
Para asegurar el rigor científico de nuestras observaciones, la Tabla \ref{tab:wilcoxon} presenta las pruebas de rangos con signo de Wilcoxon (\textit{Wilcoxon signed-rank test}) evaluadas sobre todos los datasets. Esto confirma de manera concluyente la mejora estadísticamente significativa en velocidad de entrenamiento al adoptar componentes ortogonales y aditivos, y detalla la paridad competitiva en la tasa de aciertos contra modelos consolidados del estado del arte.
...
Al aislar la influencia axial mediante la adición de polinomios independientes, el kernel GAM posee auditabilidad plena (White-Box). Extrajimos vectorialmente los pesos de dimensionalidad optimizados de \textit{Breast Cancer (30D)}. Tal y como se detalló teóricamente en la formulación del GAM, los ejes multiplicadores $w_d$ actúan como atenuadores de ruido. Podemos apreciar determinísticamente que el modelo suprimió más del 60\% del ruido diagnóstico y confió masivamente el resultado a factores morfológicos críticos sin requerir aproximaciones post-hoc opacas (como LIME o SHAP).
